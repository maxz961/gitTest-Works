...
class ChangeItem extends Component {
    ...
    onChange = (e) => {
        this.setState({ [e.target.id]: e.target.value, id: this.props.id })
    }

    closeBlock = e => {
        const {name, last, avatar, company, phone, email, value} = this.props.title
        this.setState({
            name,
            last,
            avatar,
    ...
    render() {
        ...
        return (
            ...
            <form className="ui form" onSubmit={e => e.preventDefault()}>
                <div className="field">
                    <label htmlFor="name"><h2>Редактирование контакта:</h2></label>
...
const mapDispatchToProps = (dispatch) => {
    return {
        ChangeItem: (name) => {dispatch({ type: 'CHANGE_ITEM', name: name})}
    }
}
